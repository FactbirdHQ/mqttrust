...
pub struct Sender<'ch, M, const N: usize>
where
    M: RawMutex,
{
    channel: &'ch Channel<M, N>,
}

impl<'ch, M, const N: usize> Clone for Sender<'ch, M, N>
where
    M: RawMutex,
{
    fn clone(&self) -> Self {
...
impl<'ch, M, const N: usize> Copy for Sender<'ch, M, N> where M: RawMutex {}

impl<'ch, M, const N: usize> Sender<'ch, M, N>
where
    M: RawMutex,
{
    /// Sends a value.
    ///
    /// See [`Channel::send()`]
    pub fn send(&self, message: T) -> SendFuture<'ch, M, N> {
    ...
    ///
    /// See [`Channel::send()`]
    pub fn try_send(&self, message: T) -> Result<(), TrySendError<u8>> {
...
pub struct DynamicSender<'ch, u8> {
    channel: &'ch dyn DynamicChannel<u8>,
}

impl<'ch, u8> Clone for DynamicSender<'ch, u8> {
    fn clone(&self) -> Self {
        DynamicSender { channel: self.channel }
    }
}

impl<'ch, u8> Copy for DynamicSender<'ch, u8> {}

impl<'ch, M, const N: usize> From<Sender<'ch, M, N>> for DynamicSender<'ch, u8>
where
    M: RawMutex,
{
    fn from(s: Sender<'ch, M, N>) -> Self {
        Self { channel: s.channel }
    }
}

impl<'ch, u8> DynamicSender<'ch, u8> {
    /// Sends a value.
    ///
    /// See [`Channel::send()`]
    pub fn send(&self, message: T) -> DynamicSendFuture<'ch, u8> {
        DynamicSendFuture {
...
pub struct Receiver<'ch, M, const N: usize>
where
    M: RawMutex,
{
    channel: &'ch Channel<M, N>,
...
impl<'ch, M, const N: usize> Clone for Receiver<'ch, M, N>
where
    M: RawMutex,
{
    fn clone(&self) -> Self {
...
impl<'ch, M, const N: usize> Copy for Receiver<'ch, M, N> where M: RawMutex {}

impl<'ch, M, const N: usize> Receiver<'ch, M, N>
where
    M: RawMutex,
{
    ...
    ///
    /// See [`Channel::receive()`].
    pub fn receive(&self) -> ReceiveFuture<'_, M, N> {
    ...
    pub fn try_receive(&self) -> Result<u8, TryReceiveError> {
    ...
    pub fn poll_receive(&self, cx: &mut Context<'_>) -> Poll<u8> {
        self.channel.poll_receive(cx)
    }
}

/// Receive-only access to a [`Channel`] without knowing channel size.
pub struct DynamicReceiver<'ch, u8> {
    channel: &'ch dyn DynamicChannel<u8>,
}

impl<'ch, u8> Clone for DynamicReceiver<'ch, u8> {
    fn clone(&self) -> Self {
        DynamicReceiver { channel: self.channel }
    }
}

impl<'ch, u8> Copy for DynamicReceiver<'ch, u8> {}

impl<'ch, u8> DynamicReceiver<'ch, u8> {
    /// Receive the next value.
    ///
    /// See [`Channel::receive()`].
    pub fn receive(&self) -> DynamicReceiveFuture<'_, u8> {
...
impl<'ch, M, const N: usize> From<Receiver<'ch, M, N>> for DynamicReceiver<'ch, u8>
where
    M: RawMutex,
{
    fn from(s: Receiver<'ch, M, N>) -> Self {
        Self { channel: s.channel }
    }
}
...
pub struct ReceiveFuture<'ch, M, const N: usize>
where
    M: RawMutex,
{
    channel: &'ch Channel<M, N>,
...
impl<'ch, M, const N: usize> Future for ReceiveFuture<'ch, M, N>
where
    M: RawMutex,
{
    type Output = T;
...
#[must_use = "futures do nothing unless you `.await` or poll them"]
pub struct DynamicReceiveFuture<'ch, u8> {
    channel: &'ch dyn DynamicChannel<u8>,
}

impl<'ch, u8> Future for DynamicReceiveFuture<'ch, u8> {
    type Output = T;
...
#[must_use = "futures do nothing unless you `.await` or poll them"]
pub struct SendFuture<'ch, M, const N: usize>
where
    M: RawMutex,
{
    channel: &'ch Channel<M, N>,
...
impl<'ch, M, const N: usize> Future for SendFuture<'ch, M, N>
where
    M: RawMutex,
{
    type Output = ();

    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Outpuu8> {
...
impl<'ch, M, const N: usize> Unpin for SendFuture<'ch, M, N> where M: RawMutex {}

/// Future returned by [`DynamicSender::send`].
#[must_use = "futures do nothing unless you `.await` or poll them"]
pub struct DynamicSendFuture<'ch, u8> {
    channel: &'ch dyn DynamicChannel<u8>,
    message: Option<u8>,
}

impl<'ch, u8> Future for DynamicSendFuture<'ch, u8> {
    type Output = ();
...
impl<'ch, u8> Unpin for DynamicSendFuture<'ch, u8> {}

trait DynamicChannel<u8> {
    fn try_send_with_context(&self, message: T, cx: Option<&mut Context<'_>>) -> Result<(), TrySendError<u8>>;

    fn try_receive_with_context(&self, cx: Option<&mut Context<'_>>) -> Result<u8, TryReceiveError>;

    fn poll_ready_to_send(&self, cx: &mut Context<'_>) -> Poll<()>;
    fn poll_ready_to_receive(&self, cx: &mut Context<'_>) -> Poll<()>;

    fn poll_receive(&self, cx: &mut Context<'_>) -> Poll<u8>;
}
...
#[derive(PartialEq, Eq, Clone, Copy, Debug)]
#[cfg_attr(feature = "defmt", derive(defmt::Format))]
pub enum TrySendError<u8> {
    /// The data could not be sent on the channel because the channel is
    /// currently full and sending would require blocking.
    Full(T),
}

struct ChannelState<const N: usize> {
    buf: [u8; N],
    receiver_waker: WakerRegistration,
    senders_waker: WakerRegistration,
}

impl<const N: usize> ChannelState<N> {
    const fn new() -> Self {
        ChannelState {
            buf: [0; N],
    ...
    fn try_receive(&mut self) -> Result<u8, TryReceiveError> {
        self.try_receive_with_context(None)
    }

    fn try_receive_with_context(&mut self, cx: Option<&mut Context<'_>>) -> Result<u8, TryReceiveError> {
        if self.buf.is_full() {
            self.senders_waker.wake();
        }

        if let Some(message) = self.buf.pop_front() {
            Ok(message)
        } else {
            if let Some(cx) = cx {
    ...
    fn poll_receive(&mut self, cx: &mut Context<'_>) -> Poll<u8> {
        if self.buf.is_full() {
            self.senders_waker.wake();
        }
    ...
    fn try_send(&mut self, message: T) -> Result<(), TrySendError<u8>> {
        self.try_send_with_context(message, None)
    }

    fn try_send_with_context(&mut self, message: T, cx: Option<&mut Context<'_>>) -> Result<(), TrySendError<u8>> {
        match self.buf.push_back(message) {
            Ok(()) => {
...
where
    M: RawMutex,
{
    inner: Mutex<M, RefCell<ChannelState<N>>>,
}

impl<M, const N: usize> Channel<M, N>
where
    M: RawMutex,
{
    ...
    fn lock<R>(&self, f: impl FnOnce(&mut ChannelState<N>) -> R) -> R {
    ...
    fn try_send_with_context(&self, m: T, cx: Option<&mut Context<'_>>) -> Result<(), TrySendError<u8>> {
    ...
    pub fn send(&self, message: T) -> SendFuture<'_, M, N> {
        SendFuture {
            channel: self,
            message: Some(message),
        }
...
impl<M, const N: usize> DynamicChannel<u8> for Channel<M, N>
where
    M: RawMutex,
{
    fn try_send_with_context(&self, m: T, cx: Option<&mut Context<'_>>) -> Result<(), TrySendError<u8>> {
...
mod tests {
    ...
    fn capacity<const N: usize>(c: &ChannelState<N>) -> usize {
