...
impl<'a> Property<'a> {
    ...
    pub fn property_id(&self) -> u8 {
        PropertyIdentifier::from(self) as u8
    }
}
...
impl<'a> Properties<'a> {
    ...
    pub fn iter(&self) -> impl core::iter::Iterator<Item = Result<Property<'a>, Error>> {
        let data = match self {
            Properties::Slice(data) => PropsData::Decoded(data),
            Properties::DataBlock(data) => PropsData::Encoded(data),
            _ => {
...
enum PropsData<'a> {
    Encoded(&'a [u8]),
    Decoded(&'a [Property<'a>]),
}

impl PropsData<'_> {
    fn len(&self) -> usize {
        match self {
            PropsData::Encoded(data) => data.len(),
            PropsData::Decoded(data) => data.len(),
        }
    }
}
...
impl<'a> PropertiesIter<'a> {
    ...
    fn check_remaining(&self, len: usize) -> Result<(), EncodingError> {
        match self.data {
            PropsData::Encoded(data) => {
                if data[self.index..].len() < len {
                    return Err(EncodingError::InvalidLength);
                }
            }
            PropsData::Decoded(_) => todo!(),
        }
        Ok(())
    }

    fn read_varint(&mut self) -> Result<u32, EncodingError> {
        let PropsData::Encoded(data) = self.data else {
            return Err(EncodingError::MalformedPacket);
        };

        let mut integer = 0;
        for (i, byte) in data[self.index..].iter().take(4).enumerate() {
    ...
    fn read_slice(&mut self) -> Result<&'a [u8], EncodingError> {
        ...
        let bytes = &data[self.index..self.index + len];
        self.index += len;
        Ok(bytes)
    }
    ...
    fn read_property(&mut self) -> Result<Property<'a>, EncodingError> {
        let PropsData::Decoded(data) = self.data else {
            ...
            return Ok(
                    ...
                    .map_err(|_| EncodingError::InvalidProperty(property_v))?
                {
                    ...
                    PropertyIdentifier::ServerKeepAlive => {
                        Property::ServerKeepAlive(self.read_u16()?)
                    }
                    ...
                    PropertyIdentifier::RetainAvailable => {
                        Property::RetainAvailable(self.read_u8()?)
                    }
