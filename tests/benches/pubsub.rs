...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    static mut PUBSUB: PubSubChannel<CriticalSectionRawMutex, StaticBufferProvider<65536>, 1> =
        PubSubChannel::new_static();
    let mut prod = unsafe { PUBSUB.publisher().unwrap() };
    let mut cons = unsafe { PUBSUB.subscriber().unwrap() };
...
fn chunky(data: &[u8], chunksz: usize) {
    static mut PUBSUB: PubSubChannel<CriticalSectionRawMutex, StaticBufferProvider<4096>, 1> =
        PubSubChannel::new_static();
    let mut prod = unsafe { PUBSUB.publisher().unwrap() };
    let mut cons = unsafe { PUBSUB.subscriber().unwrap() };
